...
public class DeadLetterQueueConsumer {
    ...
    public void receiveDelayedMessage(Message message, Channel channel) throws Exception {
        String msg = new String(message.getBody());
        log.info("当前时间：{}，收到延迟队列的消息：{}", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new Date()), msg);
    }
    
    // 接收发布确认消息
    @RabbitListener (queues = {MqConstant.CONFIRM_QUEUE})
